...
data        <- read.table(unz(usefile, 
                              "WDICSV.csv"),
                          header = T, 
                          quote  = "\"", 
                          sep    = ",")
topics      <- read.table(unz(usefile, 
                              "WDISeries.csv"),
                          header = T, 
                          quote  = "\"", 
                          sep    = ",") 
countrymeta <- read.table(unz(usefile, 
                              "WDICountry.csv"),
                          header = T, 
                          quote  = "\"", 
                          sep    = ",")
seriesmeta  <- read.table(unz(usefile, 
                              "WDISeries.csv"),
                          header = T, 
                          quote  = "\"", 
                          sep    = ",")
...
seriesmeta <- seriesmeta %>%
  ...
  mutate(datatopic = ifelse(Topic == "Public sector",  
                            "Public Sector",  
                            Topic),
         datatopic = ifelse(datatopic == "Private sector", 
                            "Private Sector", 
                            datatopic))
...
wdic2000 <- datal %>%
  ...
  mutate(nonmiss_tot2000 = round(100 * total_obs/(max(n_country)*(1 + current_year - 2000)), 
                                 2)) %>%
...
wdic <- datal %>%
  ...
  mutate(nonmiss     = round(100 * total_obs/(n_country*span_years), 
                             2),
         nonmiss_tot = round(100 * total_obs/(max(n_country)*max(span_years)), 
                             2)) %>%
  merge(wdic2000, 
        by = "Indicator.Code")
...
pageName = paste("en:wb:datamain:/indicator/", 
                 dat$Indicator.Code,
                 sep = "")
pageviews_visits = QueueTrended("wbgglobalprod", 
                                current_date - years(1), 
                                current_date, 
...
indicators_metascore_df <- seriesmeta %>% #count number of words in each field
  mutate(Aggregationmethod_score = if_else(is.na(Aggregation.method),
                                           as.integer(0),
                                           sapply(strsplit(Aggregation.method, 
                                                           '\\W+'), length)) ,
         Developmentrelevance_score = if_else(is.na(Development.relevance),
                                              as.integer(0),
                                              sapply(strsplit(Development.relevance, 
                                                              '\\W+'), 
                                                     length)) ,
         License_Type_score = if_else(is.na(License.Type),
                                      as.integer(0),
                                      sapply(strsplit(License.Type, 
                                                      '\\W+'), 
                                             length)) ,
         Limitationsandexceptions_score = if_else(is.na(Limitations.and.exceptions),
                                                  as.integer(0),
                                                  sapply(strsplit(Limitations.and.exceptions, 
                                                                  '\\W+'), 
                                                         length)) ,
         Longdefinition_score = if_else(is.na(Long.definition),
                                        as.integer(0),
                                        sapply(strsplit(Long.definition, 
                                                        '\\W+'), 
                                               length)) ,
         Periodicity_score = if_else(is.na(Periodicity),
                                     as.integer(0),
                                     sapply(strsplit(Periodicity, 
                                                     '\\W+'), 
                                            length)) ,
         Source_score = if_else(is.na(Source),
                                as.integer(0),
                                sapply(strsplit(Source, 
                                                '\\W+'), 
                                       length)) ,
         Statisticalconceptandmethodology_score = if_else(is.na(Statistical.concept.and.methodology),
                                                          as.integer(0),
                                                          sapply(strsplit(Statistical.concept.and.methodology, 
                                                                          '\\W+'), 
                                                                 length)) ,
         Shortdefinition_score = if_else(is.na(Short.definition),
                                         as.integer(0),
         ...
         metadata_length = Developmentrelevance_score + 
           ...
           License_Type_score + 
           Limitationsandexceptions_score + 
           Longdefinition_score + 
           Periodicity_score + 
           Source_score +
  ...
  mutate(#produce score on any availability
    Developmentrelevance_min_score = if_else(Developmentrelevance_score > 0, 
                                             1,    
                                             0),
    Statisticalconceptandmethodology_min_score = if_else(Statisticalconceptandmethodology_score > 0,
                                                         1,
                                                         0),
    License_Type_min_score = if_else(License_Type_score > 0,
                                     1,
                                     0),
    definition_min_score = if_else((Longdefinition_score + Shortdefinition_score > 0),
                                   1,
                                   0),
    Source_min_score = if_else(Source_score > 0,
                               1,
                               0),  
    #overall score
    metadata_availability = Developmentrelevance_min_score + 
      Statisticalconceptandmethodology_min_score +
      License_Type_min_score +
      definition_min_score + 
      Source_min_score,
    metadata_fail = case_when(
      ...
      Longdefinition_score + 
        Shortdefinition_score < 5                ~ "Definition missing",
...
score_table <- wdic %>%
  ...
  mutate(ncountry_score               = n_country/217*100,
         ...
         frontier_score               = (ncountry_score + 
                                           plmic_score + 
                                           yearlatest_score + 
                                           yearlatestmedian_score + 
                                           spanyears_score + 
                                           nonmiss_score + 
                                           uniquevisitors_score +
                                           metadata_availability_fscore +
                                           metadata_length_fscore)/9
  ) %>%
  mutate(
    ...
    yearlatest_median_score = case_when(
      ...
      between(yearlatest_median,
              scen$yearlatest_median[1],
              scen$yearlatest_median[2])  ~ 2,
      between(yearlatest_median,
              scen$yearlatest_median[2],
              scen$yearlatest_median[3]) ~ 3,
    ...
    uniquevisitors_score = case_when(
      ...
      between(uniquevisitors,
              scen$uniquevisitors[1],
              scen$uniquevisitors[2])  ~ 2,
      between(uniquevisitors,
              scen$uniquevisitors[2],
              scen$uniquevisitors[3]) ~ 3,
    ...
    metadata_availability_score = case_when(
      ...
      between(metadata_availability,
              scen$metadata_availability[1],
              scen$metadata_availability[2])  ~ 2,
      between(metadata_availability,
              scen$metadata_availability[2],
              scen$metadata_availability[3]) ~ 3,
    ...
    metadata_length_score = case_when(
      ...
      between(metadata_length,
              scen$metadata_length[1],
              scen$metadata_length[2])  ~ 2,
      between(metadata_length,
              scen$metadata_length[2],
              scen$metadata_length[3]) ~ 3,
  ...
  mutate( #do a Distance to Threshold approach, where the frontiers are set by the thresholds
    country_score_hybrid = case_when(
      ...
      between(n_country,
              scen$n_country[4],
              scen$n_country[1]) ~ (n_country-scen$n_country[4])/(scen$n_country[1] - scen$n_country[4]),
      between(n_country,
              scen$n_country[1],
              scen$n_country[2])  ~ 1 + 
        (n_country-scen$n_country[1])/(scen$n_country[2] - scen$n_country[1]),
      between(n_country,
              scen$n_country[2],
              scen$n_country[3]) ~ 2 + 
        (n_country-scen$n_country[2])/(scen$n_country[3] - scen$n_country[2]),
      between(n_country,
              scen$n_country[3],
              scen$n_country[5]) ~ 3 + 
        (n_country - scen$n_country[3])/(scen$n_country[5] - scen$n_country[3]),
    ...
    p_lmic_score_hybrid = case_when(
      ...
      between(p_lmic,
              scen$p_lmic[4],
              scen$p_lmic[1])  ~ (p_lmic-scen$p_lmic[4])/(scen$p_lmic[1] - scen$p_lmic[4]),
      between(p_lmic,
              scen$p_lmic[1],
              scen$p_lmic[2])  ~ 1 + (p_lmic - scen$p_lmic[1])/(scen$p_lmic[2] - scen$p_lmic[1]),
      between(p_lmic,
              scen$p_lmic[2],
              scen$p_lmic[3]) ~ 2 + 
        (p_lmic - scen$p_lmic[2])/(scen$p_lmic[3] - scen$p_lmic[2]),
      between(p_lmic,
              scen$p_lmic[3],
              scen$p_lmic[5]) ~ 3 + 
        (p_lmic - scen$p_lmic[3])/(scen$p_lmic[5] - scen$p_lmic[3]),
      p_lmic > scen$p_lmic[5] ~ 4),
    yearlatest_median_score_hybrid = case_when(
      yearlatest_median < scen$yearlatest_median[4] ~ 0,
      between(yearlatest_median,
              scen$yearlatest_median[4],
              scen$yearlatest_median[1]) ~ (yearlatest_median - scen$yearlatest_median[4])/(scen$yearlatest_median[1] - scen$yearlatest_median[4]),
      between(yearlatest_median,
              scen$yearlatest_median[1],
              scen$yearlatest_median[2]) ~ 1 + (yearlatest_median - 
                                                  scen$yearlatest_median[1])/(scen$yearlatest_median[2] - 
                                                                                scen$yearlatest_median[1]),
      between(yearlatest_median,
              scen$yearlatest_median[2],
              scen$yearlatest_median[3]) ~ 2 + 
        (yearlatest_median - scen$yearlatest_median[2])/(scen$yearlatest_median[3] - scen$yearlatest_median[2]),
      between(yearlatest_median,
              scen$yearlatest_median[3],
              scen$yearlatest_median[5]) ~ 3 + 
        (yearlatest_median - scen$yearlatest_median[3])/(scen$yearlatest_median[5] - scen$yearlatest_median[3]),
      yearlatest_median > scen$yearlatest_median[5] ~ 4),
    yearlatest_score_hybrid = case_when(
      yearlatest < scen$yearlatest[4] ~ 0,
      between(yearlatest,
              scen$yearlatest[4],
              scen$yearlatest[1])  ~ (yearlatest - scen$yearlatest[4])/(scen$yearlatest[1] - scen$yearlatest[4]),
      between(yearlatest,
              scen$yearlatest[1],
              scen$yearlatest[2])  ~ 1 + (yearlatest-scen$yearlatest[1])/(scen$yearlatest[2]-scen$yearlatest[1]),
      between(yearlatest,
              scen$yearlatest[2],
              scen$yearlatest[3]) ~ 2 + (yearlatest - scen$yearlatest[2])/(scen$yearlatest[3] - scen$yearlatest[2]),
      between(yearlatest,
              scen$yearlatest[3],
              scen$yearlatest[5]) ~ 3 + 
        (yearlatest - scen$yearlatest[3])/(scen$yearlatest[5] - scen$yearlatest[3]),
      yearlatest > scen$yearlatest[5] ~ 4),
    span_years_score_hybrid = case_when(
      span_years < scen$span_years[4] ~ 0,
      between(span_years,
              scen$span_years[4],
              scen$span_years[1])  ~ (span_years - scen$span_years[4])/(scen$span_years[1] - scen$span_years[4]),
      between(span_years,
              scen$span_years[1],
              scen$span_years[2])  ~ 1 +
        (span_years - scen$span_years[1])/(scen$span_years[2]-scen$span_years[1]),
      between(span_years,
              scen$span_years[2],
              scen$span_years[3]) ~ 2 +
        (span_years-scen$span_years[2])/(scen$span_years[3]-scen$span_years[2]),
      between(span_years,
              scen$span_years[3],
              scen$span_years[5]) ~ 3 + 
        (span_years - scen$span_years[3])/(scen$span_years[5] - scen$span_years[3]),
      span_years > scen$span_years[5] ~ 4),
    uniquevisitors_score_hybrid = case_when(
      uniquevisitors < scen$uniquevisitors[4] ~ 0,
      between(uniquevisitors, 
              scen$uniquevisitors[4],
              scen$uniquevisitors[1]) ~ (uniquevisitors - scen$uniquevisitors[4])/(scen$uniquevisitors[1] - scen$uniquevisitors[4]),
      between(uniquevisitors, 
              scen$uniquevisitors[1],
              scen$uniquevisitors[2])  ~ 1 + (uniquevisitors - scen$uniquevisitors[1])/(scen$uniquevisitors[2] - scen$uniquevisitors[1]),
      between(uniquevisitors,
              scen$uniquevisitors[2],
              scen$uniquevisitors[3]) ~ 2 + (uniquevisitors - scen$uniquevisitors[2])/(scen$uniquevisitors[3] - scen$uniquevisitors[2]),
      between(uniquevisitors,
              scen$uniquevisitors[3],
              scen$uniquevisitors[5]) ~ 3 + (uniquevisitors - scen$uniquevisitors[3])/(scen$uniquevisitors[5] - scen$uniquevisitors[3]),
      uniquevisitors > scen$uniquevisitors[5] ~ 4)   ,
    nonmiss_score_hybrid = case_when(
      nonmiss < scen$nonmiss[4] ~ 0,
      between(nonmiss,
              scen$nonmiss[4],
              scen$nonmiss[1])  ~ (nonmiss-scen$nonmiss[4])/(scen$nonmiss[1]-scen$nonmiss[4]),
      between(nonmiss,
              scen$nonmiss[1],
              scen$nonmiss[2])  ~ 1+(nonmiss-scen$nonmiss[1])/(scen$nonmiss[2]-scen$nonmiss[1]),
      between(nonmiss,
              scen$nonmiss[2],
              scen$nonmiss[3]) ~ 2+(nonmiss-scen$nonmiss[2])/(scen$nonmiss[3]-scen$nonmiss[2]),
      between(nonmiss,
              scen$nonmiss[3],
              scen$nonmiss[5]) ~ 3 + (nonmiss - scen$nonmiss[3])/(scen$nonmiss[5] - scen$nonmiss[3]),
      nonmiss > scen$nonmiss[5] ~ 4) ,
    metadata_availability_score_hybrid = case_when(
      nonmiss < scen$metadata_availability[4] ~ 0,
      between(metadata_availability,
              scen$metadata_availability[4],
              scen$metadata_availability[1]) ~ (metadata_availability - scen$metadata_availability[4])/(scen$metadata_availability[1]-scen$metadata_availability[4]),
      between(metadata_availability,
              scen$metadata_availability[1],
              scen$metadata_availability[2]) ~ 1 + 
        (metadata_availability - scen$metadata_availability[1])/(scen$metadata_availability[2] - scen$metadata_availability[1]),
      between(metadata_availability,
              scen$metadata_availability[2],
              scen$metadata_availability[3]) ~ 2 + 
        (metadata_availability - scen$metadata_availability[2])/(scen$metadata_availability[3] - scen$metadata_availability[2]),
      between(metadata_availability,
              scen$metadata_availability[3],
              scen$metadata_availability[5]) ~ 3 + 
        (metadata_availability - scen$metadata_availability[3])/(scen$metadata_availability[5] - scen$metadata_availability[3]),
    ...
    metadata_length_score_hybrid = case_when(
      ...
      between(metadata_length,
              scen$metadata_length[4],
              scen$metadata_length[1])  ~ (metadata_length-scen$metadata_length[4])/(scen$metadata_length[1]-scen$metadata_length[4]),
      between(metadata_length,
              scen$metadata_length[1],
              scen$metadata_length[2])  ~ 1+(metadata_length-scen$metadata_length[1])/(scen$metadata_length[2]-scen$metadata_length[1]),
      between(metadata_length,
              scen$metadata_length[2],
              scen$metadata_length[3]) ~ 2+(metadata_length-scen$metadata_length[2])/(scen$metadata_length[3]-scen$metadata_length[2]),
      between(metadata_length,
              scen$metadata_length[3],
              scen$metadata_length[5]) ~ 3 + (metadata_length-scen$metadata_length[3])/(scen$metadata_length[5]-scen$metadata_length[3]),
  ...
  mutate(loose = if_else( #calculate whether indicator would fall in loose, median, or stringent as well
    (n_country                < scen$n_country[1] |
       ...
       metadata_length        < scen$metadata_length[1]),
    "Yes", 
    "No"),
    median = if_else(
      (n_country                < scen$n_country[2] |
         ...
         metadata_length        < scen$metadata_length[2]),
      "Yes", 
      "No"),
    stringent = if_else(
      (n_country                < scen$n_country[3] |
         ...
         metadata_length        < scen$metadata_length[3]),
      "Yes", 
      "No"),) %>%      
  mutate(
    threshold_score = (country_score + 
                         p_lmic_score +
                         yearlatest_median_score +
                         yearlatest_score + 
                         span_years_score + 
                         uniquevisitors_score + 
                         nonmiss_score +
                         metadata_availability_score + 
                         metadata_length_score)/9,
    
    hybrid_score = (country_score_hybrid +
                      p_lmic_score_hybrid +
                      yearlatest_median_score_hybrid +
                      yearlatest_score_hybrid + 
                      span_years_score_hybrid + 
                      uniquevisitors_score_hybrid + 
                      nonmiss_score_hybrid +
                      metadata_availability_score_hybrid +
                      metadata_length_score_hybrid)/9,) %>%
  mutate(
    ...
    metadata_score       = (metadata_availability_score + 
                              metadata_length_score)/2,
    threshold_score_wgtd = (geographic_score +
                              temporal_score +
                              completeness_score +
                              usage_score +
                              metadata_score)/5) %>%   
  mutate(
    ...
    temporal_score_hybrid     = (yearlatest_median_score_hybrid +
                                   yearlatest_score_hybrid + 
                                   span_years_score_hybrid)/3,
    completeness_score_hybrid = nonmiss_score_hybrid,
    usage_score_hybrid        = uniquevisitors_score_hybrid,
    metadata_score_hybrid     = (metadata_availability_score_hybrid +
                                   metadata_length_score_hybrid)/2,
    hybrid_score_wgtd         = (geographic_score_hybrid + 
                                   temporal_score_hybrid + 
                                   completeness_score_hybrid + 
                                   usage_score_hybrid + 
                                   metadata_score_hybrid)/5,
    hybrid_score_wgtd2        = (geographic_score_hybrid +
                                   temporal_score_hybrid +
                                   usage_score_hybrid +
