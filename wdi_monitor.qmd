...
var_df <- 
  data.frame(var_selection = var_selection, 
             var_descr     = var_descr)
box_vars_df <- var_df %>%
  filter(var_selection %in% c("countryobs_avg", 
                              "span_years", 
                              "cov_years", 
                              "p_lmic"))
...
```{r}
# Title for the four tiles
htmltools::div(
  style = "width: 100%; text-align: center; padding: 2px; background-color: #FFFFFF; margin-bottom: 2px;",
  htmltools::h4("Median (for typical WDI Topic)"))
```
...
htmltools::div(
  class = "custom-card",
  style = paste0("background-color: ", plmics_color, "; color: white; padding: 20px;"),
  htmltools::a(
    href = "#license",
    style = card_style,
    # htmltools::tags$i(
...
htmltools::div(
  class = "custom-card",
  style = paste0("background-color: ", lyr_color, "; color: white; padding: 20px;"),
  htmltools::a(
    href = "#license",
    style = card_style,
    # htmltools::tags$i(
...
htmltools::div(
  class = "custom-card",
  style = paste0("background-color: ", syr_color, "; color: white; padding: 20px;"),
  htmltools::a(
...
htmltools::div(
  class = "custom-card",
  style = paste0("background-color: ", nctry_color, "; color: white; padding: 20px;"),
  htmltools::a(
    href = "#country-coverage",
    style = card_style,
    # htmltools::tags$i(
...
htmltools::div(
  style = "width: 100%; text-align: center; padding: 2px; background-color: #FFFFFF; margin-bottom: 2px;",
  htmltools::h4("Overview of selected metrics"))
```
...
```{r}
# Create a new Plotly object for the boxplots
p_box <- plot_ly()

for (i in seq_len(nrow(box_vars_df))) {
  var_nm    <- box_vars_df$var_selection[i]  # e.g. "yearfirst"
  var_label <- box_vars_df$var_descr[i]      # e.g. "First Year"
  
  # 1) Extract the numeric data for this column
  y_values <- score_table[[var_nm]]
  
  # 2) Add a box trace for these values, labeling the box with var_label
  p_box <- p_box %>%
    add_trace(
      x    = rep(var_label, length(y_values)),  # single "category" for this variable
      y    = y_values,
      type = "box",
      name = var_label
    )
