...
#   }
#   
#   # We initialize its name as e.g. "No indicator selected" or "None"
# p_strip <- p_strip %>%
#   add_trace(
#     x = c(NA, NA),
#     y = c(NA, NA),
#     mode = "lines",
#     line = list(color = "red"),
#     name = "None",           # Legend label before any indicator is selected
#     showlegend = TRUE
#   )
#   
...
# }
# 
# # 2) X-axis: categorical, custom order
# x_layout <- list(
#   type          = "category",
#   categoryorder = "array",
#   categoryarray = metric_names
# )
# 
# # 3) Function to create shapes (lines) + annotations (values on lines + bottom-left code)
# make_indicator_artifacts <- function(ind_code, row_i, metrics, metric_names) {
#   
#   n_metrics  <- length(metrics)
#   shape_list <- vector("list", n_metrics)
#   annot_list <- vector("list", n_metrics)
#   
#   # Build lines + numeric annotations
#   for (idx in seq_len(n_metrics)) {
#     val <- row_i[[ metrics[idx] ]]
#     if (!is.na(val) && !is.null(val)) {
#       
...
#       )
#       
#       # Just the numeric value near the line
#       text_lab <- paste0(round(val, 1))
#       annot_list[[idx]] <- list(
#         x         = x1 + 0.05,
#         y         = val,
...
#   }
#   
#   # ~~~~~~~~~ EXTRA ANNOTATION: "Indicator: <code>" at bottom-left
#   # We place it in "paper" coordinates so it pins to the overall plot, not the data scale
#   # e.g., x=0.02, y=0.02 => near bottom-left corner
#   corner_annot <- list(
#     x         = -0.25,
#     y         = -0.045,
#     xref      = "paper",
#     yref      = "paper",
#     text      = paste0("Indicator: ", ind_code),
#     xanchor   = "left",
#     yanchor   = "bottom",
#     showarrow = FALSE,
#     font      = list(color = "red"))
#   
#   # Append that extra annotation
#   annot_list[[n_metrics + 1]] <- corner_annot
#   
#   # Filter out any NULL shapes/annotations
#   shape_list <- shape_list[!sapply(shape_list, is.null)]
#   annot_list <- annot_list[!sapply(annot_list, is.null)]
#   
#   list(
#     shapes      = shape_list,
#     annotations = annot_list
...
# }
# 
# # 4) Build dropdown: each indicator => shapes + annotations + bottom-left code
# indicators <- unique(score_table$Indicator.Code)
# 
...
#     slice(1)
#   
#   artifacts <- make_indicator_artifacts(ind, row_i, metrics, metric_names)
#   shape_list <- artifacts$shapes
#   annot_list <- artifacts$annotations
#   
#   list(
#     label  = ind,
#     method = "relayout",  # We'll just override layout with shapes+annotations
#     args   = list(list(
#       shapes      = shape_list,
#       annotations = annot_list
#     ))
...
#     type   = "dropdown",
#     active = 0,
#     buttons = c(list(none_button), buttons)
#   )
# )
# 
# # 5) Final layout
...
make_indicator_artifacts <- function(ind_code, ind_name, row_i, metrics, metric_names) {
  
  # 'row_i' is a 1-row df containing columns: Indicator.Code, Indicator.Name, plus your 4 metrics
  n_metrics  <- length(metrics)
  shape_list <- vector("list", n_metrics)
  annot_list <- vector("list", n_metrics)
  ...
  # 2) OPTIONAL: If you want a bottom-left annotation for the indicator code:
  bottom_left_annot <- list(
    x = 0,
    y = -0.075,
    xref = "paper",
    yref = "paper",
    text = paste0("Indicator: ", ind_name),
    xanchor = "left",
    yanchor = "bottom",
    showarrow = FALSE,
  ...
  # 3) NEW SUBTITLE at the top, showing the 'Indicator.Name' from row_i
  indicator_name <- row_i$Indicator.Name
  subtitle_annot <- list(
    x = 0.5,
    y = 1.08,
    xref = "paper",
    yref = "paper",
    text = paste0(indicator_name, " highlighted in red"),
    xanchor = "middle",
    yanchor = "bottom",
    showarrow = FALSE,
    font = list(color = "black")
  )
  annot_list[[n_metrics + 2]] <- subtitle_annot
